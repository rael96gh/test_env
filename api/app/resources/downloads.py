...
@downloads_ns.route('/primer_pooling_file')
class DownloadPrimerPoolingFile(Resource):
      @downloads_ns.expect(download_primers_csv_model)
      @downloads_ns.doc('download_primer_pooling_file')
      def post(self):
          """Download primer pooling file as CSV"""
          try:
              data = request.get_json()
              generated_primers = data.get('generated_primers')
              generated_oligos = data.get('generated_oligos')
              well_format = data.get('well_format')
              dest_well_format = data.get('dest_well_format')

              # The sequence_name parameter from the utility function is not used for multiple sequences.
              # We pass an empty string as a placeholder.
              sequence_name = ""
              ...
              return Response(
                  ...
                  headers={"Content-disposition":
                           "attachment; filename=primer_pooling.csv"})
          except Exception as e:
              print(f"Error in primer pooling file download: {e}")
              return {'error': f'Error generating primer pooling file: {str(e)}'}, 500
